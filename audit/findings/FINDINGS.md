...
| F-039 | Unknown product categories fall through to country wildcard tax rules instead of returning 422 | **25** | `audit/qa/runs/RUN-001.md` QA-001 | rule resolution | human/build agent | **OPEN** |
| F-040 | Route-level validation failures are absent from the immutable audit trail | **20** | `audit/qa/runs/RUN-001.md` QA-002 | audit invariant | human/build agent | **OPEN** |
| F-041 | Documented demo command leaves all tracked compliance reports modified | docs | `audit/qa/runs/RUN-001.md` QA-003 | clean clone | human/build agent | **OPEN** |
...
### F-039 — OPEN 2026-08-05T09:11Z — Unknown categories are silently taxed
...
### F-040 — OPEN 2026-08-05T09:14Z — Rejected requests bypass audit persistence

A production missing-amount request with fixed transaction id returned
structured 400, then its audit URL returned 404. This breaks the invariant that
every request, including failures, is persisted. QA-002 has the exact evidence.

### F-041 — OPEN 2026-08-05T09:09Z — Clean clone is dirty after the demo

All documented commands succeed, but `npm run demo` rewrites the five tracked
compliance reports. `git status --short` is therefore not clean after a reviewer
follows the README. This has persisted across 14 daily probes.
